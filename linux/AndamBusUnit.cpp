...
void AndamBusUnit::doWork() {
    ...
    try {
      ...
      if (ret != 0) {
        ...
        AB_DEBUG("Different address " << e.getFrameAddress() << " this is " << e.getTargetAddress());
    } catch (SynchronizationLostException &e) {
        AB_WARNING("SynchronizationLostException:" << e.what());
        bs.trySync();
    } catch (BusTimeoutException &e) {
        AB_DEBUG("DoWork.BusTimeoutException:" << e.what());
    }
#endif // ARDUINO
}
...
//        LOG_U("Create port");
    ...
    case AndamBusCommand::NONE:
        ...
        AB_ERROR("NONE received");
        return;
    case AndamBusCommand::GET_DATA:
    case AndamBusCommand::VIRTUAL_DEVICE_CHANGE:
    case AndamBusCommand::VIRTUAL_ITEMS_REMOVED:
        AB_ERROR("Command " << commandString(cmd.command) << " not implemented");
#ifdef ARDUINO
		const __FlashStringHelper* cs = commandString(cmd.command);
		if (cs != nullptr) {
			AB_ERROR(F("Command ") << cs << F(" not implemented"));
		}
		else {
			AB_ERROR(F("Command ") << (int)cmd.command << F(" not implemented"));
		}
#endif
    ...
    AB_ERROR("Command x " << (int)(cmd.command) << " not implemented");
    responseError(AndamBusCommandError::NOT_IMPLEMENTED);
}
...
//        AB_ERROR("sendRawData " << (i==cnt-1) << " " << cfgSize);
...
//    AB_ERROR("sending property " << sizeof(ItemProperty));
}
...
//    AB_ERROR("startresponse sending " << sizeof(AndamBusFrameHeader) << " + " << size);
    bs.startFrame(size, 0, counter);

    if (respHdrSize > 0) {
//        AB_ERROR("continue sending " << respHdrSize);
